...
describe('MySongsPage', () => {
  ...
  it('counts the songs in each section', async () => {
    renderMySongsPage([
      ...
      buildUserSong({id: 11, song: buildSong({spotifyId: 'spotify-b'}), status: 'WANT_TO_LEARN'})
    ])

    expect(
      await screen.findByRole('heading', {level: 2, name: /want to learn/iu})
    ).toHaveTextContent('2')
    expect(screen.getByRole('heading', {level: 2, name: /learned/iu})).toHaveTextContent('0')
  })

  it('puts each song under its own status, in section order', async () => {
    renderMySongsPage([
      buildUserSong({id: 10, song: buildSong({title: 'Wanted'}), status: 'WANT_TO_LEARN'}),
      buildUserSong({
        id: 11,
