...
function BackLink() {
  ...
  return (
    <Link className='self-start text-slate-400 text-sm hover:text-slate-200' to='/'>
      &larr; All songs
    </Link>
  )
}
  ...
  return (
    <dl className='w-full text-sm'>
      ...
      <DetailRow
        label='Difficulty'
        value={
          userSong.difficultyRating === null ? 'Not rated' : `${userSong.difficultyRating} / 5`
        }
      />
      <DetailRow label='Added' value={dateFormatter.format(userSong.dateAdded)}/>
    </dl>
  )
}
...
function SongActions({
  ...
  if (userSong !== undefined) {
    return (
      <StatusControl isPending={isUpdating} onChange={onStatusChange} status={userSong.status}/>
    )
  }

  if (!isLoggedIn) {
    return (
      <Link className='rounded-sm bg-blue-500 p-2 text-center hover:bg-blue-400' to='/login'>
...
function SongDetails({spotifyId}: SongDetailsProps) {
  ...
  const seed = v.safeParse(SpotifySong, (location.state as { song?: unknown } | null)?.song)
  ...
  const userSong = userSongsQuery.data?.find(entry => entry.song.spotifyId === spotifyId)
  ...
  return (
    <div className='flex flex-col items-center p-4'>
      ...
      <div className='flex w-full max-w-md flex-col gap-y-4'>
        ...
        <AlbumArt
          album={song.album}
          className='aspect-square w-full self-center rounded-sm shrink-0'
