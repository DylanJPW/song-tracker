...
describe('StatusControl', () => {
	...
	it('marks only the current status as pressed', () => {
		render(
			<StatusControl isPending={false} onChange={vi.fn()} status='LEARNING' />
		)

		expect(
			screen.getByRole('button', {name: 'Learning', pressed: true})
		).toBeInTheDocument()
		expect(
			screen.getByRole('button', {name: 'Want to learn', pressed: false})
		).toBeInTheDocument()
		expect(
			screen.getByRole('button', {name: 'Learned', pressed: false})
		).toBeInTheDocument()
	})
